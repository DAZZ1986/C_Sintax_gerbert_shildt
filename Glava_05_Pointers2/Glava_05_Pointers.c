...
int main(void)
{
    ...
    */
    ...
    Если оба указателя имеют один и тот же тип, то выполняется простое присваивание, без преобразования типа. В 
    следующем примере
    #include <stdio.h>
    ...
    выводит адреса в формате, используемом компилятором. А значениями указателея являются присвоенные им адреса в памяти!!
    ...
    (мой коммент - если указатель интовый, то и ссылаться должен на инты, а не на даблы, потому что, переменная int занимает в 
    памяти 4 байта, а double — 8 байтов!)
    ...
    Короче, ниже попытка дотянуться дт значения типа дабл, через интовый указатель!!!
    #include <stdio.h>
    int main(void)
    {
        double x = 100.1, y;
    ...
    передаст переменной y только 4 байта информации, а не 8 байтов, необходимых для double. Несмотря на то, что p 
    ссылается на объект double, оператор присваивания выполнит действие с объектом типа int, потому что p объявлен 
    как указатель на int. Поэтому такое использование указателя p неправильное.
    ...
    p1++;   //смещение будет происходить на колитчество байт того типа на который он указывает, если тип инт то 
    ...
    p1 = p1 + 12; //тут еденица измерения тоесть 12 = колву байт типа на который ссылаемся как писли выше! Тоесть если
    ...
    "передвигает" указатель p1 на 12 объектов в сторону увеличения адресов. //ЖЕНЯ - а как мы уверены что далее в памяти
            //будут теже типы данных а не другие?? - Ответ - если ты смещаешся то ты должзен понимать куда смещаешься, 
    ...
    */
    ...
    int *p = 0;
    *p = 10; //ошибка! Тут операция разитменовывания - тоесть я пытаюсь изменить переменную на которую указывает указатель.
    
    p = 10; //в данном случае я пеперазисал значение указателя с 0 на 10, тоесть он смотрит на адрес в памяти 10.
    *p = 10; //а, в данном случае я пытаюсь присвоить значение 10 в переменную на которую указывает указатель, а 
    ...
    В этом случае присваивание посредством p будет присваиванием по нулевому адресу, что обычно вызывает разрушение 
    программы.

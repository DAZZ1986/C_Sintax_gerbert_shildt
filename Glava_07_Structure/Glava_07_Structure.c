...
int main(void)
{
    ...
    с помощью:
    1. структуры(struct) — группы переменных, имеющей одно имя и называемой агрегатным типом данных. (Кроме того, еще известны
       термины соединение (compound) и конгломерат (conglomerate).);
    2. объединения(union) - которое позволяет определять один и тот же участок памяти как два или более типов переменных;
    ...
    4. перечисления(enum) — списка поименованных целых констант;
    ...
    */
    ...
    передаются функциям через стек!!!!!!!!!!!) Впрочем, для простых структур с несколькими членами эти ресурсы являются не 
    такими уж большими. Но если в структуре имеется большое количество членов или некоторые члены сами являются массивами, 
    то при передаче структур функциям производительность может упасть до недопустимо низкого уровня. Как решить эту проблему? 
    Надо передавать не саму структуру, а указатель на нее.      // !!!!!!!!!!!!!!!!!!!!!!!!!

    // !!!!!!!!!!!!!!!!!!!!!!!!!
    Когда функции передается указатель на структуру, то в стек попадает только адрес структуры!!!!!!!! В результате вызовы 
    функции выполняются очень быстро. В некоторых случаях этот способ имеет еще и второе преимущество: передача указателя 
    позволяет функции модифицировать содержимое структуры, используемой в качестве аргумента!!!!!!!!
    ...
    p = &person;    //определение указателя на структуру, присваеваем адрес члена структуры указателю.   // !!!!!!!!!!!!!!!!!
    ...
    struct my_time {    //объявили глобальную структуру
        int hours;
        int minutes;
        int seconds;
    };
    ...
    int main(void)
    {
        struct my_time systime;   //объявление члена структуры
    ...
    */
    ...
    printf("\n\n\n/////////// Объединения (union) \n");
    /*
    Объединение — это место в памяти, которое используется для хранения переменных, разных типов!!!!!!!!!!! Объединение 
    дает возможность интерпретировать один и тот же набор битов не менее, чем двумя разными способами. Объявление объединения
    (начинается с ключевого слова union) похоже на объявление структуры и в общем виде выглядит так:
    union тег {
      тип имя-члена;
      тип имя-члена;
      тип имя-члена;
    ...
    написать отдельный оператор объявления. Пример:
    union u_type cnvt;      // !!!!!!!!!!!!!!!!!!!!!!!!!

    В cnvt одну и ту же область памяти занимают целая переменная i и символьная переменная ch!!!!!!!!!!!!!!! Конечно, i 
    занимает 2 байта(при условии, что целые значения занимают по 2 байта), a ch — только 1. На рис. 7.2 показано, каким 
    образом i и ch пользуются одним и тем же адресом. В любом месте программы хранящиеся в cnvt данные можно обрабатывать
    как целые или символьные.         // !!!!!!!!!!!!!!!!!!!!!!!!!
    ...
    ЖЕНЯ ответ - нет, работа в памяти происходит перетеранием, поэтому для union память выделяется исходя из максимального
    элемента, в данном примере это 2 байтовая i !
    ...
    void func1(union u_type *un)    //тут в функцию передается указатель на член cnvt типа union    // !!!!!!!!!!!!!!!!!!
    {
    ...
    union pw {
        short int i;
        char ch[2];//ЖЕНЯ почему тут двухбайтовые символы подразумеваются? если в чар 1 байт и там массив из 3 эл. по 1 байту!?
    };
    ...
    */
    ...
    бы 3 байта.       Каким образом?  // !!!!!!!!!!!!!!!!!!!!!!!!!    ТУТ НУЖНО УТОЧНИТЬ ЖЕНЯ
    ...
    */
    ...
    присваиваются значения, большие предшествующего.    // !!!!!!!!!!!!!!!!!!!!!!!!!
    
    Например, следующий код присваивает quarter значение 100:
    ...
    char name[][12] = {     //ЖЕНЯ - это массив, двумерный??
      "пенни",
      "никель",
      "монета в 10 центов",
      "четверть доллара",
    ...
    */
    ...
    когда нужно знать размер структуры, следует использовать sizeof. // !!!!!!!!!!!!!!!!!!!!!!!!!
    
    Например, если требуется динамически выделять память для объекта типа s, необходимо использовать последовательность 
    операторов, аналогичную той, что показана здесь (а не вставлять вручную значения длины его членов):
    struct s *p;            // !!!!!!!!!!!!!!!!!!!!!!!!!
    ...
    */
    ...
    для объединенний берется размер наибольшиго члена объединения!
    */
    ...
    balance over_due;   //создали переменную типа float             // !!!!!!!!!!!!!!!!!!!!!!!!!

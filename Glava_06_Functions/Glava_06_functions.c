...
int main(void)
{
    ...
    */
    ...
    char* xgets(char* s)  //char* ОЗНАЧАЕТ ЧТО тип возвращающего значения должен быть указателем, как у нас 
                          //и происходит, это указатель *p и его мы возвращаем(return p) из функции xgets.
    {
        char ch, *p;
        int t;
    ...
    */  
    ...
    применения. 
        - Во-первых, он обеспечивает немедленный выход из функции, т.е. заставляет выполняющуюся программу передать
          управление коду, вызвавшему функцию. 
        - Во-вторых, этот оператор можно использовать для того, чтобы возвратить значение. 
    ...
    // !!!!!!!!!!!!!!!!!!!!!!!!!
    ПЕРВЫЙ способ используется тогда, когда после выполнения последнего оператора в функции встречается закрывающая 
    фигурная скобка }. 
    Например, функция pr_reverse() в приведенной ниже программе просто выводит на экран в обратном порядке строку 
    "Мне нравится С", а затем возвращает управление вызывающей программе.
    #include <string.h>
    #include <stdio.h>
    ...
    туда, откуда она была вызвана. Но на практике не так уж много функций используют именно такой способ завершения 
    выполнения. 
    
    // !!!!!!!!!!!!!!!!!!!!!!!!!
    ВТОРОЙ способ - большинство функций для завершения выполнения используется оператор return — или потому, что необходимо
    вернуть значение, или чтобы сделать код функции проще и эффективнее.
    ...
    ///////////Возврат значений
    // !!!!!!!!!!!!!!!!!!!!!!!!!
    Все функции, кроме тех, которые относятся к типу void, возвращают значение. Это значение указывается после оператора  
    return. Стандарт С89 допускает выполнение оператора return без указания возвращаемого значения, в функциях отличных
    от void. В этом случае все равно происходит возврат какого-нибудь произвольного значения. Но такое положение 
    ...
    закрывающей ее фигурной скобки), то возвращается произвольное значение. Хотя здесь нет синтаксической ошибки, это 
    является серьезным упущением и таких ситуаций необходимо избегать.
    ...
    x = power(y);   // !!!!!!!!!!!!!!!!!!!! тоесть функция может что то вернуть, чтобы это чтото присвоить воему оператору
    ...
    ни целыми, ни целыми без знака. Они являются адресами в памяти и относятся к особому типу данных!!!!!!!!!!!!!!!!!!!
    Такая особенность указателей определяется тем, что арифметика указателей(адресная арифметика) работает с учетом 
    параметров базового типа.
    ...
    * // !!!!!!!!!!!!!!!!!!!!!!!!!
    ...
    int main(void)
    {
        ...
        x = 10;
        sqr_it(x);  //несоответствие типов, тк в функцию передаем целое, вместо указателя на целое.
    ...
    float f(void);          // !!!!!!!!!!!!!!!!!!!!!!!!! С возвращающим значением и без параметров
    или
    void f2(void);          // !!!!!!!!!!!!!!!!!!!!!!!!! БЕЗ возвращающего значения и без параметров
    ...
    double div();  //старомодное объявление функции, где не писались параметры

...
----------
    ...
    */
    ...
    функции _getch() и getch() были несовместимы с функциями ввода стандартного С, например, с функциями scanf_s() или gets().
    Поэтому вам придется вместо стандартных функций использовать такие их специальные версии, как cscanf_s() или cgets(). 
    ...
    */
    ...
    printf("\n\n\n/////////// Чтение и запись строк\n");
    /*
    Среди функций ввода/вывода на консоль есть и более сложные, но и более мощные: это функции gets() и puts(), которые 
    позволяют считывать и отображать строки символов.

    Функция gets() читает строку символов, введенную с клавиатуры, и записывает ее в память по адресу, на который указывает
    ее аргумент. Символы можно вводить с клавиатуры до тех пор, пока не будет введен символ возврата каретки. Он не станет
    частью строки, а вместо него в ее конец будет помещен символ конца строки ('0'), после чего произойдет возврат из 
    функции gets(). На самом деле вернуть символ возврата каретки с помощью этой функции нельзя (а с помощью getchar() 
    — как раз можно). Перед тем как нажимать <ENTER>, можно исправлять неправильно введенные символы, пользуясь для этого 
    клавишей возврата каретки на одну позицию (клавишей backspace). Вот прототип для gets():
    char *gets(char *cmp);

    Здесь cmp — это указатель на массив символов, в который записываются символы, вводимые пользователем, gets() также
    возвращает cmp. Следующая программа читает строку в массив str и выводит ее длину:
    #include <stdio.h>
    #include <string.h>
    ...
    char* dic[][40] = {     //ЖЕНЯ - что лежит в массиве указателей?
    ...
    */
    ...
    printf("\n\n\n/////////// Форматный ввод / вывод на консоль\n");
    /*
    Функции printf() и scanf_s() выполняют форматный ввод и вывод, то есть они могут читать и писать данные в разных форматах. 
    Данные на консоль выводит printf(). А ее "дополнение", функция scanf_s(), наоборот — считывает данные с клавиатуры. Обе 
    ...
    */
    ...
    %p	Выводит указатель
    %n	Аргумент, соответствующий этому спецификатору, должен быть указателем на целочисленную переменную. Спецификатор 
    позволяет сохранить в этой переменной количество записанных символов (записанных до того места, в котором находится код %n)
    %%	Выводит знак %
    ...
    поддерживать те же спецификаторы, которые применяются в функции scanf_s().
    ...
    что ширина поля вывода будет не менее 10 символов, причем для десятичных разрядов будет отведено четыре позиции(после точки).
    ...
    int main(void)
    {
        ...
        printf("%10.8d\n", 1000); //тут первая цифра означает сколько вообще символов будет занимать строка, а вторая цифра говорит
    ...
    */
    ...
    printf("\n\n\n/////////// scanf_s()\n");
    /*
    Функция scanf_s() — это программа ввода общего назначения, выполняющая ввод с консоли. Она может читать данные всех 
    встроенных типов и автоматически преобразовывать числа в соответствующий внутренний формат, scanf_s() во многом выглядит
    как обратная к printf(). Вот прототип функции scanf_s():

    int scanf_s(const char *управляющая_строка, ...);

    Эта функция возвращает количество тех элементов данных, которым было успешно присвоено значение. В случае ошибки scanf_s()
    ...
    Каждый спецификатор формата ввода начинается со знака %, причем спецификаторы формата ввода сообщают функции scanf_s() 
    ...
    Таблица 8.3. Спецификаторы преобразования для функции scanf_s() 
    Код	Значение
    ...
    Функцию scanf_s() можно использовать для чтения целых значений в восьмеричной или шестнадцатеричной форме, применяя для 
    ...
    Функция scanf_s() прекращает чтение числа тогда, когда встречается первый нечисловой символ.
    ...
    ///////////Чтение одиночных символов с помощью scanf_s()
    Как уже говорилось в этой главе, одиночные символы можно прочитать с помощью функции getchar() или какой-либо функции, 
    родственной с ней. Для той же цели можно использовать также вызов функции scanf_s() со спецификатором формата %c. Но, как
    и большинство реализаций getchar(), функция scanf_s() при использовании спецификатора преобразования %c обычно будет 
    ...
    других типов разделители интерпретируются как разделители полей. Например, при вводе с входного потока "x y" фрагмент
    кода scanf_s("%c%c%c", &a, &b, &c);
    ...
    ///////////Чтение строк
    Для чтения из входного потока строки можно использовать функцию scanf_s() со спецификатором преобразования %s. 
    Использование спецификатора преобразования %s заставляет scanf_s() читать символы до тех пор, пока не встретится какой-либо
    разделитель. Читаемые символы помещаются в символьный массив, на который указывает соответствующий аргумент, а после 
    введенных символов еще добавляется символ конца строки ('0'). Что касается scanf_s(), то таким разделителем может быть 
    пробел, разделитель строк, табуляция, вертикальная табуляция или подача страницы. В отличие от gets(), которая читает 
    строку, пока не будет нажата клавиша <ENTER>, scanf_s() читает строку до тех пор, пока не встретится первый разделитель.
    Это означает, что scanf_s() нельзя использовать для чтения строки "это испытание", потому что после пробела процесс 
    ...
    заставляет функцию scanf_s() читать адрес в том формате, который определен архитектурой центрального процессора.
    ...
    ///////////Спецификатор %n
    Спецификатор %n указывает, что scanf_s() должна поместить количество символов, считанных (до того момента, когда 
    ...
    Функция scanf_s() поддерживает спецификатор формата общего назначения, называемый набором сканируемых символов (scanset). 
    Набор сканируемых символов представляет собой множество символов. Когда scanf_s() обрабатывает такое множество, то 
    ...
    квадратной скобкой должен находиться знак %. Например, следующий набор сканируемых символов дает указание scanf_s() 
    сканировать только символы X, Y и Z:
    %[XYZ]

    При использовании набора сканируемых символов функция scanf_s() продолжает читать символы, помещая их в соответствующий
    массив символов, пока не встретится символ, не входящий в этот набор. При возвращении из scanf_s() в массиве символов 
    ...
    случае 't' не входит в набор сканируемых символов, то scanf_s() прекратила чтение символов в переменную str сразу после
    ...
    наборе должен быть ^. Этот символ дает указание scanf_s() принимать любой символ, который не входит в набор сканируемых
    символов.

    В большинстве реализаций для указания диапазона можно использовать дефис. Например, указанный ниже набор сканируемых 
    символов дает функции scanf_s() указание принимать символы от А до Z:
    %[A-Z]
    ...
    Разделитель в управляющей строке дает scanf_s() указание пропустить в потоке ввода один или несколько начальных 
    разделителей. Разделителями являются пробелы, табуляции, вертикальные табуляции, подачи страниц и разделители строк. 
    В сущности, один разделитель в управляющей строке заставляет scanf_s() читать, но не сохранять любое количество (в том 
    ...
    Если в управляющей строке находится символ, не являющийся разделителем, то функция scanf_s() прочитает символ из входного 
    потока, проверит, совпадает ли прочитанный символ с указанным в управляющей строке, и в случае совпадения пропустит 
    прочитанный символ. Например, "%d,%d" заставляет scanf_s() прочитать целое значение, прочитать запятую и пропустить ее 
    (если это была запятая!), а затем прочитать следующее целое значение. Если же указанный символ во входном потоке не 
    будет найден, то scanf_s() завершится. Когда нужно прочитать и отбросить знак процента, то в управляющей строке следует 
    указать %%.
    
    
    
    
    ///////////Функции scanf_s() необходимо передавать адреса
    Для всех переменных, которые должны получить значения с помощью scanf_s(), должны быть переданы адреса. Это означает, 
    ...
    Как и printf(), функция scanf_s() дает возможность модифицировать некоторое число своих спецификаторов формата. В 
    ...
    где оно закончилось при предыдущем вызове. Например, если вы в ответ на вызов scanf_s() из этого примера введете
    ABCDEFGHIJKLMNOPRSTUVWXYZ

    то в str из-за спецификатора максимальной ширины поля будет помещено только 20 символов, то есть символы вплоть до Т. 
    Это значит, что оставшиеся символы UVWXYZ пока еще не прочитаны. При следующем вызове scanf_s(), например при выполнении
    оператора

    scanf_s("%s", str);

    в str будут помешены буквы UVWXYZ. Ввод из поля может завершиться и до того, как будет достигнута максимальная длина 
    поля — если встретится разделитель. В таком случае scanf_s() переходит к следующему полю.
    ...
    По умолчанию спецификаторы f, e и g дают scanf_s() указание присваивать данные переменной типа float. Если перед одним 
    из этих спецификаторов будет помещен l (эль), то scanf_s() будет присваивать данные переменной типа double. Использование 
    L дает scanf_s() указание, чтобы переменная, принимающая данные, имела тип long double.
    ...
    На заметку: В С99 для функции scanf_s() имеются еще и другие модификаторы типа; о них рассказывается в части II.
    ...
    ///////////Подавление ввода
    scanf_s() может прочитать поле, но не присваивать прочитанное значение никакой переменной; для этого надо перед 
    литерой-спецификатором формата поля поставить звездочку, *. Например, когда выполняется оператор
    scanf_s("%d%*c%d", &x, &y);   //ЖЕНЯ не понимаю зачем столько процентов, нужна ли звездочка в принтф

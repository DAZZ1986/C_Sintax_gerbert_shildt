...
int main(void)
{
    ...
    */
    ...
    #define - определение нового имени
    #error - заставляет компилятор прекратить компиляцию и эта директива используется в основном для отладки.
    #include - команда компилятору для компиляции вставленного файла

    Директивы условной компиляции:
        #if - начало условной конструкции
        #elif - часть условной конструкции
        #else - часть условной конструкции
        #endif - конец условной конструкции

        #ifdef - компилируется если имя макроса определено в операторе #define
        #ifndef - компилируется если имя макроса еще НЕ определено в операторе #define

    #undef - удаляет ранее заданное определение имени макроса

    #defined - второй способ узнать, определено ли имя макроса после #ifdef.
    #line - изменение нумерации строки кода, так же применяется для отладки итд.
    #pragma - позволяет передавать компилятору различные инструкции. Например, компилятор может поддерживать трассировку
    ...
    */
    ...
    "if defined" (если определено) и "if not defined" (если не определено). 
    
    В общем виде #ifdef выглядит таким образом:
    #ifdef имя_макроса
        последовательность операторов
    #endif
    ...
    */
    ...
    сочетании с оператором времени компиляции defined!!!!!!!!!!!
    В общем виде оператор defined выглядит таким образом:
    defined имя_макроса

    Если имя_макроса определено, то выражение считается истинным; в противном случае — ложным. // !!!!!!!!!!!!!!!!!!!!!!!
    
    // !!!!!!!!!!!!!!!!!!!!!!!
    Например, чтобы узнать, определено ли имя макроса MYFILE, можно использовать одну из двух команд препроцессора:
    #if defined MYFILE
    или
    #ifdef MYFILE
    ...
    */
    ...
    компиляторе:
        - В первом из них содержится номер компилируемой в данный момент строки кода. 
        - А второй идентификатор — это строка, содержащая имя компилируемого исходного файла. 
    ...
    это любой допустимый идентификатор файла, становящийся новым значением __FILE__. 
